...
// до того, как игрок упрётся в несуществующую сцену или битую картинку.

const { chapters, meta, scenes } = gameData;
...
describe("identifiers", () => {
  ...
  test("every scene belongs to an existing chapter or to the endings", () => {
    const orphans = scenes
      .filter((scene) => scene.chapter !== 0 && !chapterIds.has(scene.chapter))
    ...
    const stuck = scenes
      .filter(
        (scene) =>
          !scene.isEnding &&
          !scene.choices.some(
            (choice) => !choice.conditions || choice.conditions.length === 0,
          ),
      )
      .map((scene) => scene.id);
