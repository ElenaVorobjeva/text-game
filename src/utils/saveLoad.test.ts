...
import { clearAllSaves, loadGame } from "./saveLoad";
...
describe("loadGame: saved progress", () => {
  ...
  test("keeps each game's save separate", () => {
    writeProgress(GAME, makeState({ currentSceneId: "chapter1_scene1" }));
    writeProgress(OTHER, makeState({ currentSceneId: "chapter3_scene1" }));
...
describe("loadGame: content drift", () => {
  test("refuses a save pointing at a scene that no longer exists", () => {
    ...
    writeProgress(
      GAME,
      makeState({ currentSceneId: "chapter1_scene_removed" }),
    );
...
describe("loadGame: validates against the passed engine", () => {
  test("accepts a scene that exists in the given game but not the bundled one", () => {
    ...
    writeProgress(GAME, makeState({ currentSceneId: "other_start" }));
